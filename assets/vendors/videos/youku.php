            ...
            foreach ($segs as $key=>$val) {
                if(in_array($key,$streamtypes)){
                    foreach($val as $k=> $v){
                        ...
                        $data['videos'][$sharpness[$key]][$k] = array(
                            "type" => $fileType,
                            "url" => "http://k.youku.com/player/getFlvPath/sid/".$sid."_00/st/{$fileType}/fileid/".$fileId."?K=".$_k."&hd=1&myp=0&ts=".((((($v['seconds'].'&ypp=0&ctype=12&ev=1&token=').$token).'&oip=').$ip).'&ep=').$ep,
                        );
                    }
                }
            }
...
$cachePool = $app['cache_factory']('videoyouku');
$item = $cachePool->getItem($id);
if (!$item->isMiss()) {
    $json = $item->get();
} else {
    $json = json_encode(Youku::parse('http://v.youku.com/v_show/id_' . $_GET['id'] . '.html'));
